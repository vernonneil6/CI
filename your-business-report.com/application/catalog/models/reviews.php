...
class Reviews extends CI_Model
{
	...
	//Inserting Record
	function insert($companyid,$userid,$rating,$review)
	{
		$siteid = $this->session->userdata('siteid');
	...
	function insert1($companyid,$userid,$rating,$review)
	{
		$siteid = $this->session->userdata('siteid');
		$date = date_default_timezone_set('Asia/Kolkata');
		$date = date('Y-m-d H:i:s');
	...
	function get_review_byid($id)
 	{
	//	$siteid = $this->session->userdata('siteid');
		$this->db->select('r.*, cm.company,cm.logo,cm.aboutus,u.firstname,u.username,u.lastname,u.avatarbig,u.gender');
		$this->db->from('reviews as r');
		$this->db->join('company as cm','r.companyid=cm.id','left');
		$this->db->join('user as u','r.reviewby=u.id','left');
	...
	//Updating Record
	function update($id,$companyid,$userid,$rating,$review)
 	{
		$date = date('Y-m-d H:i:s');
		$varipaddress = $_SERVER['REMOTE_ADDR'];
		$data = array(
	...
	function insert_elite_review($companyid,$userid,$rating,$review)
	{
		$siteid = $this->session->userdata('siteid');
		$date = date_default_timezone_set('Asia/Kolkata');
		$date = date('Y-m-d H:i:s');
