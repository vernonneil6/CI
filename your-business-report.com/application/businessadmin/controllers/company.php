		...
		if( $this->session->userdata['youg_admin'] )
	  	{
				...
				if( $this->input->post('id') )
				{
					...
					$creditcard1 = addslashes($this->input->post('creditcard1'));
					$creditcard2 = addslashes($this->input->post('creditcard2'));
					//Loading Model File
					$this->load->model('companys');
						...
						if(count($_FILES['companylogo'])>0)
						{
							if( $_FILES['companylogo']['name']!='' && $_FILES['companylogo']['size'] > 0 )
							{
								...
								if( count($error)==0 && count($imgdata) > 0 )
								{
									...
									if( $this->companys->update($id,$company,$streetaddress,$city,$state,$country,$zip,$email,$siteurl,$paypalid,$imgdata['file_name'],$phone,$about,$category,$creditcard1,$creditcard2) )
									{
							...
							else
							{
								...
								if( $this->companys->update_noimage($id,$company,$streetaddress,$city,$state,$country,$zip,$email,$siteurl,$phone,$about,$category,$creditcard1,$creditcard2) )
								{
									$this->session->set_flashdata('success', 'company updated successfully.');
									redirect('company', 'refresh');
								}
