...
class Login extends CI_Controller {
	...
	*/
	...
	{  
	  if(!$this->session->userdata('youg_user') )
	  { 
		  if( ($this->input->post('email') && $this->input->post('password') )|| ($this->input->post('loginemail') && $this->input->post('loginpassword')))
		  {
			...
			$user_email = $this->input->post('loginemail');
			$user_pass = $this->input->post('loginpassword');
			}
			//query the database
	...
	public function update()
	{
		...
		if($this->input->post('forgottype') == 'fusername')
		{
			$site_name = $this->common->get_setting_value(1);
			$site_url = $this->common->get_setting_value(2);
...
';
						  ...
						  else
						  {
							 ...
							 redirect('login/forgotusername', 'refresh');
						  }
			...
			{
			  $this->session->set_flashdata('error', 'The e-mail address was not found in our records, please try again!');
			  redirect('login/forgotusername', 'refresh');
			}
	...
	/*					  $site_logo = "<a href='".$site_url."' title='".$site_name."'><img src='".base_url()."images/logo.png' border='0' width='241' height='83' border='0' alt='".$site_name."'></a>";*/
						  ...
						  if($this->email->send())
						  {
							$this->session->set_flashdata('success', 'A new password has been sent to your e-mail address.');
