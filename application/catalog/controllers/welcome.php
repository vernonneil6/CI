	...
	public function __construct()
  	...
  	parent::__construct();
		...
		if (preg_match('/(?P<domain>[a-z0-9][a-z0-9\-]{1,63}\.[a-z\.]{2,6})$/i', $domain, $regs))
		 ...
		 {
		 	$this->data['title'] = 'Have a Complaint? Report It and Get It Resolved! ';
		 }
...
public function updates($id)
	{
		...
		if( $this->input->post('btnsubmit') || $this->input->post('detail')!='')
		{
			...
			//$damagesinamt = ($this->input->post('damagesinamt'));
	...
	public function update()
	{
		...
		if( $this->input->post('btnsubmit') || $this->input->post('detail')!='')
		{
			...
			$company1 = ($this->input->post('company'));
			$company = strtolower($company1);
			$username = ($this->input->post('username'));
			...
			if(count($elitemem)>0)
			{
				if(count($relation)>0)
				{
						if($relation[0]['status']=='Decline')
							{
								if($this->complaints->insert($type,$companyid,$userid,$damagesinamt,$whendate,$location,$detail,$username,$emailid,$statusdisable='yes',$city,$state,$zip,$company ,$reasondispute,$merchantresolution,$streetaddress,$phone,$caseid,$transactionid,$transaction_date,$transactionamt,$complaintdate))
								{
						...
						{		
						if($updated = $this->complaints->insert($type,$companyid,$userid,$damagesinamt,$whendate,$location,$detail,$username,$emailid,'yes',$city,$state,$zip,$company ,$reasondispute,$merchantresolution,$streetaddress,$phone,$caseid,$transactionid,$transaction_date,$transactionamt,$complaintdate))
						{	
					$com = $this->complaints->get_company_byid($companyid);
				...
				{
					if($updated = $this->complaints->insert($type,$companyid,$userid,$damagesinamt,$whendate,$location,$detail,$username,$emailid,$statusdisable='yes',$city,$state,$zip,$company ,$reasondispute,$merchantresolution,$streetaddress,$phone,$caseid,$transactionid,$transaction_date,$transactionamt,$complaintdate))
				{	
					$com = $this->complaints->get_company_byid($companyid);
					$user = $this->common->get_user_byid($userid);
	...
	'>Decline Customer</a>";
			...
			{
				if($updated = $this->complaints->insert($type,$companyid,$userid,$damagesinamt,$whendate,$location,$detail,$username,$email,'no',$city,$state,$zip,$company ,$reasondispute,$merchantresolution,$streetaddress,$phone,$caseid,$transactionid,$transaction_date,$transactionamt,$complaintdate))
				{	
					$com = $this->complaints->get_company_byid($companyid);
	...
	/*public function update()
	{
		...
		if( $this->input->post('btncompany'))
		{
			...
			$data=array(
				...
				'caseid'=>$this->input->post('caseid'),
				'transactionid'=>$this->input->post('transid'),
				'transaction'=>$this->input->post('transamt'),
				'transaction_date'=>$this->input->post('transdate'),
				'complaintdate'=>$this->input->post('complaintdate'),
				'reasondispute'=>$this->input->post('reasondispute'),
				'merchantresolution'=>$this->input->post('merchantresolution'),
				...
				'damagesinamt'=>$this->input->post('damagesinamt'),
				'whendate'=>$this->input->post('whendate'),
				'location'=>$this->input->post('location')
				//'categoryid'=>$this->input->post(''),
				//'registerdate'=>$this->input->post(''),
				//'registerip'=>$this->input->post(''),
				//'status'=>$this->input->post(''),
				//'websiteid'=>$this->session->userdata('siteid'),
				//'userid'=>$this->session->userdata['youg_user']['userid'],
				//'complainip'=>$_SERVER['REMOTE_ADDR']
			);
			$this->complaints->insert_data($data);
		}
	}*/
