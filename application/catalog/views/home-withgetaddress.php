...
<div class="header_wrapper">
    ...
    <nav class="navbar" role="navigation">
        ...
        <ul class="top_row">
            ...
            <div class="small">
                  <?php if(!empty($lastprice)) echo number_format($lastprice,4);?>
                </div>
          </li>
              <li class="price_box">
            <div class="big">low price</div>
            <div class="small">
                  <?php if(!empty($lowprice)) echo number_format($lowprice,4);?>
                </div>
          </li>
              <li class="price_box last_child">
            <div class="big">high price</div>
            <div class="small">
                  <?php if(!empty($highprice)) echo number_format($highprice,4);?>
                </div>
          </li>
            </ul>
        <ul class="bottom_row">
            ...
            <div class="small"><?php echo number_format($dailychange,4);?></div>
          </li>
              <li class="price_box">
            <div class="big">range</div>
            <div class="small"><?php if(!empty($lowprice)) echo number_format($lowprice,4);?>- <?php if(!empty($highprice)) echo number_format($highprice,4);?></div>
          </li>
              <li class="price_box last_child">
            <div class="big">24h volume</div>
            <div class="small"><?php if(!empty($vol24)) echo number_format($vol24,4);?></div>
          </li>
            </ul>
      </div>
        ...
        <ul class="nav navbar-top-links navbar-right">
            ...
            <div class="balance_head">Current<br>
                  Balance</div>
            <div class="usd">$ <span class="usddata"><?php echo number_format($balance=$this->homes->get_userbalance('USD'),4);?></span> USD</div>
            <div class="btc">$ <span class="btcdata"> <?php echo number_format($balance=$this->homes->get_userbalance('BTC'),4);?></span> BTC</div>
          </div>
              <div class="currency">
             <?php $currencydata=$this->common->get_currency_top();?>
            <select id="currencytop" name="currencytop">
            	<?php  for($i=0;$i<count($currencydata);$i++){?>
                  <option value="<?php echo $currencydata[$i]['currencycode'];?>" title="<?php echo $currencydata[$i]['currencyname'];?>"><?php echo $currencydata[$i]['currencycode'];?></option>
                  <?php }?>
     ...
     <ul class="dropdown-menu dropdown-messages" style="margin-top:15px;max-height: 400px;overflow-y: auto;">  
		  ...
		  {?>
            ...
            <!-- /.dropdown-messages --> 
              ...
              <!-- /.dropdown -->
               <?php $get_notifications=$this->common->get_notifications();?>
              <li class="dropdown" style="margin-top:15px"> <a class="dropdown-toggle" data-toggle="dropdown" href="#" title="notification"> <i><img src="images/bell.png" alt=""></i><span class="badge" id="notcount"><?php echo count($get_notifications);?></span> </a>
            <ul class="dropdown-menu dropdown-alerts">
               <?php 
			   if(count($get_notifications)>0){
			   		for($i=0;$i<count($get_notifications);$i++){
			   ?>
             	  <li id="<?php echo $get_notifications[$i]['unqid'];?>"> <div><strong><?php echo $get_notifications[$i]['title'];?></strong><span class="pull-right text-muted"><a href="javascript:void(0);" style="float:right" onclick="marknotification(<?php echo $get_notifications[$i]['unqid'];?>)">Mark as Read.</a></span><br />
 </div><div style="float:left"><span class="pull-right text-muted"><em><?php echo $get_notifications[$i]['notification_message'];?></em></span></div></li>
    <li class="divider"></li>
               <?php }} else {?>
                <li class="divider"></li>
				<li>No Alerts</li>
...
<?php  if( array_key_exists('cc_user',$this->session->userdata) )
    ...
    <?php $fromcurr=$this->homes->get_currency_name($exchangedata[0]['fromcurrencyid']); $fromcurr[0]['currencycode'];?>
    <?php  $tocurr=$this->homes->get_currency_name($exchangedata[0]['tocurrencyid']); $tocurr[0]['currencycode'];?>
    <?php if($fromcurr[0]['currencycode']=='BTC' || $tocurr[0]['currencycode']=='BTC'){?>
    <?php $userid = $this->session->userdata['cc_user']['userid'];
	 $btcadd = $this->homes->get_user_address($userid,'btc');
	?>
          <script type="text/javascript">
		 $(document).ready(function()
		 { 
		 			  <?php if(strlen($btcadd)>0){?>

			 setInterval(function() {
			  <?php /*?>$.ajax({
                      type 				: "POST",
                      url 					: "home/getbtcaddr/<?php echo $btcadd; ?>",
                      data				:	{},
                      dataType 			: "json",
                     // cache			   	: false,
...
{
	var sellbal=document.getElementById('sellbal').value;
	var baseprice=document.getElementById('sellbasecurrency').value
	document.getElementById('sellamount').value=sellbal;
	var total=parseFloat(sellbal)*parseFloat(baseprice);
...
{
	var buybal=document.getElementById('buybal').value;
	var baseprice=document.getElementById('buyamount').value;
	var baseprice2=document.getElementById('buybasecurrency').value;
	var final=(parseFloat(buybal)/parseFloat(baseprice2));
	var total=(parseFloat(buybal)*parseFloat(baseprice2));
	document.getElementById('buyamount').value=final;
	$('.buytotal').html((buybal));
	document.getElementById('buytotaltext').value=buybal;
}
...
$(document).ready(function() {		
		...
		//	{
				...
				document.getElementById('sellbasecurrency').value='<?php if($avg_sell==''){echo $exchangedata[0]['currentprice'];} else {echo $avg_sell;}?>';
					...
					var tocurrency=document.getElementById('currencynames1').value;
					if(sellview==0)
					{
					...
					$.ajax({
						url:urlsell,
						data:{base_price:base_price,buyamount:buyamount,buytotaltext:buytotaltext,feescalculationtext:feescalculationtext,buybasecurrency:buybasecurrency,interfaceexid:interfaceexid,currency:currency,tocurrency:tocurrency},
						type:"POST",
						success:function(data)
						{
							 if(data=="yes")
...
<script type="text/javascript">
	$(document).ready(function() {		
		...
		//	{
			...
			var tocurrency=document.getElementById('currencynameb1').value;
    ...
    <div class="section1">
        ...
        <ul>
              ...
              <li class="value_button"><a href="home/index/<?php echo $exchanges[$i]['interfaceexid'];?>" <?php if(($i==0) ){?>class="current"<?php }?> title="<?php $fromcurr=$this->homes->get_currency_name($exchanges[$i]['fromcurrencyid']); echo $fromcurr[0]['currencycode'];?> / <?php $tocurr=$this->homes->get_currency_name($exchanges[$i]['tocurrencyid']); echo $tocurr[0]['currencycode'];?>">
                ...
                <div id="<?php echo $exchanges[$i]['interfaceexid'];?>" class="small"><?php echo round($exchanges[$i]['currentprice'],4);?></div>
                </a> </li>
              <?php } else {?>
              <li class="value_button"><a href="home/index/<?php echo $exchanges[$i]['interfaceexid'];?>" <?php if(($this->uri->segment(3)==$exchanges[$i]['interfaceexid']) ){?>class="current"<?php }?> title="<?php $fromcurr=$this->homes->get_currency_name($exchanges[$i]['fromcurrencyid']); echo $fromcurr[0]['currencycode'];?> / <?php $tocurr=$this->homes->get_currency_name($exchanges[$i]['tocurrencyid']); echo $tocurr[0]['currencycode'];?>">
                ...
                <div class="small" id="<?php echo $exchanges[$i]['interfaceexid'];?>"><?php echo round($exchanges[$i]['currentprice'],4);?></div>
                </a> </li>
              <?php  }}}}?>
            </ul>
      </div>
        ...
        <div class="buy_box">
            ...
            <tbody>
                ...
                <td ><div class="mainbuy">
					...
					<input type="hidden" id="currencynameb1" name="currencynameb1"  class="checkno" value="<?php echo $fromcurr[0]['currencycode'];?>"></td>
            ...
            <tbody>
                ...
                <td>Amount
                      ...
                      <input type="hidden" id="currencyname" name="currencyname"  class="checkno" value="<?php echo $fromcurr[0]['currencycode'];?>"></td>
                       <input type="hidden" id="currencynames1" name="currencynames1"  class="checkno" value="<?php echo $tocurr[0]['currencycode'];?>"></td>
...
<span style="font-size:10px; font-style:italic">(Fees Will be <?php if(!empty($fees)) echo $fees;?>% each transaction)</span>
    ...
    width: 48%; min-height:100px !important">
        ...
        <?php }?>
         ...
         <?php $footerpages = $this->common->get_page_footer();?>
    ...
    <div class="services_part">
     ...
     <?php
        ...
        <a href="page/<?php echo $footerpages[$i]['uniquename'];?>" title="Read More">
            <li>
          <h3 class="services_head"><i> <img src="<?php echo $this->config->item('footericon_upload_path')."/";?><?php echo $footerpages[$i]['footerlogo'];?>" />
</i><?php echo $footerpages[$i]['heading'];?></h3>
          <p><?php echo substr(($footerpages[$i]['shortdesc']),0,250).'...';?></p>
        </li>
    </a>
        <?php } ?>
...
<?php if(count($exchangedata)>0){?>
   <script type="text/javascript">
	 $(document).ready(function()
	 {
		 ...
		 var id2=$('#currencycode1').val();
		 setInterval(function() {
	  $.ajax({
		  url:"home/getbalancefrom",
		  data:{id:'<?php echo $fromcurr[0]['currencycode'];?>'},
		  type:"POST",
		  success:function(d)
		  {
		  ...
		  data:{id:'<?php echo $tocurr[0]['currencycode'];?>'},
		  type:"POST",
		  success:function(d)
		  {
	 ...
	 setInterval(function() {
			  $.ajax({
					...
					$('.usddata').html($.number(data.usd,4));
					$('.btcdata').html($.number(data.btc,4));
                     $('#<?php echo $exchangedata[0]['interfaceexid'];?>').html($.number(data.exid,4))  ;
						// alert(data.balance);
			    ...
			    $.ajax({
                     ...
                     // cache			   	: false,
                      success			: function(data){
						for(var i=0; i<data.length;i++)
						{
						   $('#'+data[i].interfaceexid).html($.number(data[i].currentprice,4))  ;
...
<?php if( array_key_exists('cc_user',$this->session->userdata) )
    ...
    <?php
		...
		$userid1 = $this->session->userdata['cc_user']['userid'];
		$btcadd =  $this->homes->get_user_address($userid1,'BTC');
		$ltcadd =  $this->homes->get_user_address($userid1,'LTC');
	//	$dogeadd = $this->users->get_user_address($userid1,'DOGE');
		$peeradd = $this->homes->get_user_address($userid1,'PEERCOIN');
		  ?>
			  <?php if(strlen($btcadd)==0){ ?>
					$('#demo1div').empty();
					$('#demo1div').load('code/btc');
			  <?php  } ?>
			  
			  <?php if(strlen($ltcadd)==0){ ?>
					$('#demo2div').empty();
					$('#demo2div').load('code/ltc');
			  <?php  } ?>
			  
			<?php /*?>  <?php if(strlen($dogeadd)==0){ ?>
					$('#demo3div').empty();
					$('#demo3div').load('code/doge');
			  <?php  } ?>
			  <?php */?>
			  <?php if(strlen($peeradd)==0){ ?>
					$('#demo4div').empty();
					$('#demo4div').load('code/peercoin');
			  <?php  } ?>
      <?php } ?>
...
<?php echo $footer;?>  
<div id="demo1div" style="display:none"></div>
<div id="demo2div" style="display:none"></div>
<div id="demo3div" style="display:none"></div>
<div id="demo4div" style="display:none"></div>  
