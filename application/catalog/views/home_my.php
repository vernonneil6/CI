   ...
   <?php $check_order = $this->homes->check_atleast_one_order($ex_id);
    ...
    <script type="text/javascript">
			function drawVisualization() {
				...
				<?php for($k=1;$k<10;$k++){
			$values = $this->homes->get_values_for_graphs_today(261,$k);
			if(count($values)==5)
			{
				...
				var data = google.visualization.arrayToDataTable([
				<?php for($k=1;$k<8;$k++){
			$values = $this->homes->get_values_for_graphs_week(261,$k);
			if(count($values)==5)
			{
				$a = $values[2]['base_price'];
				$b = $values[4]['initial_price'];
			...
			$a = $values[2]['base_price'];
				...
				var data = google.visualization.arrayToDataTable([
				<?php for($k=1;$k<13;$k++){
			$values = $this->homes->get_values_for_graphs_month(261,$k);
			if(count($values)==5)
			{
				$a = $values[2]['base_price'];
				$b = $values[4]['initial_price'];
