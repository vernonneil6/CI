...
if(!empty($_COOKIE['jamcom']) && !empty($subscription['auth_transreponse_key']) && $subscription['discountcodetype'] != '30-days-free-trial'){
	$getdata = file_get_contents('http://www.yougotrated.com/affiliates/sale/amount/' . trim($subscription['amount']) . '/trans_id/' . trim($subscription['auth_transreponse_key']) . '/tracking_code/' . $_COOKIE['jamcom'] . '/customer_name/' . trim(urlencode($company['company'])));
