    ...
    <div class="section">
      ...
      <div class="serial_part">18eQACr2RqSLEKNKjW3sBKCJY2MDNFKUPP</div><?php */?>
        ...
        <div class="btable_head">
              <div class="head_balance" style="width:200px !important">Balance</div>
              <div class="head_order" style="width:140px !important">In Orders</div>
              <div class="head_total" style="width:120px !important">Status</div>
              <div class="head_total" style="width: 100px;">P(D)</div>
              <div class="head_total" style="width: 100px;">P(W)</div>
              <div class="head_total" style="width: 120px;;">Total</div>
            </div>
        <?php if(count($balance)>0){
			for($i=0;$i<count($balance);$i++){
				...
				//echo "<pre>";
				//total deposite
				$total_pending_depo = $this->users->get_pending_depo($balance[$i]['currencycode']);
				$total_pending_with = $this->users->get_pending_with($balance[$i]['currencycode']);
				
			?>
        <style>
           	.body_total{width:99px !important;}
           </style>
        <div class="btable_body1">
              <div class="body_icon"><img src="images/btc.png" alt="" title="btc"></div>
              <div class="body_balance" style="width:200px !important"><?php echo $balance[$i]['currencycode'];?> :
            ...
            <span id="btctotalbal" style="width:140px !important"></span>
            <?php
			  }else{?>
            <span id="<?php echo $balance[$i]['currencycode'];?>_Balance"> <?php echo round($balance[$i]['total_balance'],4);?> </span>
            <?php } ?>
          </div>
              <div class="body_orders" style="width:140px !important" id="<?php echo $balance[$i]['currencycode'];?>_In_Orders"> <?php echo round($totalbuyorders,4);?></div>
              <div class="body_orders" style="width:100px !important;" >
            <?php if($balance[$i]['currencycode']=='BTC'){?>
            ...
            <?php }else
              ...
              <div class="body_total" style="width:100px !important;" id="<?php echo $balance[$i]['currencycode'];?>_depo"><?php echo $total_pending_depo;?> </div>
              <div class="body_total" style="width:100px !important;" id="<?php echo $balance[$i]['currencycode'];?>_with"> <?php echo $total_pending_with;?> </div>
              <div class="body_total" style="width:100px !important;">
            <?php if($balance[$i]['currencycode']=='BTC'){?>
            <span id="btctotalbal1"></span>
            <?php }else{?>
            <span id="<?php echo $balance[$i]['currencycode'];?>Balance_After_Execution">
			
			<?php echo round(($finalbal = $balance[$i]['total_balance']-$totalbuyorders + $total_pending_depo - $total_pending_with),4);?>
...
<div class="modal fade" id="depositModal" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    margin-right: 3%;width: 94%;padding:10px !important;">
		...
		if(count($carddata)>0)
		{?>
            <label for="otplabel">Choose Credit Card <a href="user/add_card" title="Add New Card" style="text-decoration:underline;">Add New Card</a> </label>
            <br>
            <div class="tys" style="float:left !important;">
...
function update_all_balance(){
			...
			var depo = data['balance'][i]['depo'];
			var withd = data['balance'][i]['with'];
			
			//last total
			var After_Execution = data['balance'][i]['After_Execution'];
			...
			$('#'+currencycode+'_depo').empty();
			$('#'+currencycode+'_with').empty();
			
			$('#'+currencycode+'_Balance').append(total_balance);
			$('#'+currencycode+'_In_Orders').append(total);
			$('#'+currencycode+'_Balance_After_Execution').append(After_Execution);
			
			$('#'+currencycode+'_depo').append(depo);
			$('#'+currencycode+'_with').append(withd);
