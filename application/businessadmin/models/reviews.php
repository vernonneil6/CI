...
class Reviews extends CI_Model
{
	...
	function reviewsSearch($keyword, $companyid, $siteid, $limit, $offset, $sort_by, $sort_order) {
		...
		if($siteid!='all')
		{
			$q = $this->db->select('r.id,r.reviewtitle,r.reviewby,r.comment,r.rate,r.reviewdate,r.status,r.seoslug,c.company,c.logo,u.username,u.firstname,u.lastname')
			->from('reviews as r')
			...
			$q = $this->db->select('r.reviewtitle,r.reviewby,r.comment,r.rate,r.reviewdate,r.status,r.eoslug,c.company,c.logo,u.username,u.firstname,u.lastname')
			->from('reviews as r')
			->join('user as u','r.reviewby=u.id','left')
			->join('company as c','c.id=r.companyid','left')
