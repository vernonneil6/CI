...
class Home extends CI_Controller {
	...
	*/
	...
	//	else
		{
			if( array_key_exists('cc_user',$this->session->userdata) )
	  		{
				$this->data['fees']=$this->common->get_fees_user();
				//sets default or user fees
				if($this->data['fees']=='')
				{
				$this->data['fees']=$this->common->get_fees();
				}
				else if($this->data['fees']=='')
				{
	...
	//	$this->data['range']=$this->homes->get_range($intid);
		...
		$this->load->view('homecf',$this->data);
...
public function sent_otpcode()
{
		...
		$infid = $this->session->userdata('interfaceid');
		if($infid==0)
		{
			$smtpport = $this->common->get_setting_value(16);
			$SmtpServer = $this->common->get_setting_value(17);
			$SmtpUser = $this->common->get_setting_value(18);
			$SmtpPassword = $this->common->get_setting_value(19);
			$site_mail = $this->common->get_setting_value(5);
		}
		else
		{
			$smtpport = $this->common->get_interface_setting_value_bynameanduid('smtpport',$infid);
			$SmtpServer = $this->common->get_interface_setting_value_bynameanduid('SmtpServer',$infid);
			$SmtpUser = $this->common->get_interface_setting_value_bynameanduid('SmtpUser',$infid);
			$SmtpPassword = $this->common->get_interface_setting_value_bynameanduid('SmtpPassword',$infid);
			$site_mail = $this->common->get_interface_setting_value_bynameanduid('E-Mail',$infid);
			...
			if(count($user)>0)
			{
				...
				$loginsecretkey=$user[0]['loginsecretkey'];
					...
					$list = $email;
					$this->email->to($list);
					$this->email->subject($this->common->get_setting_value(1)." : ".'Transaction OTP CODE');
					...
					$mail_body=( '<p>Dear&nbsp; '.$username.',</p>
<p>Please use following OTP code for changing password <a href="'.$site_url.'" title="'.$site_name.'">'.$site_name.'</a> .</p>
<p>
	-------------------------------------------------------------------</p>
<p>
	CODE ID&nbsp; &nbsp;&nbsp; :&nbsp; '.$code.'</p>
...
{
	if($this->input->post('message')!='' && strlen($this->input->post('message'))>0)
	{
...
public function qrcode($url='test',$userid='1')
{
	
	echo '<img src="https://chart.googleapis.com/chart?chs=160x160&cht=qr&chl=mauliksuthar&choe=UTF-8" />';
}

public function loadbuydata()
{
	$intid=$this->input->post('id');
	if($intid!='')
	{
		$this->data['buyorderlist']=$this->homes->get_buyorders1($intid);
		echo json_encode($this->data['buyorderlist']);
		
	}
}

public function loadselldata()
{
	$intid=$this->input->post('id');
	if($intid!='')
	{
		$this->data['sellorderlist']=$this->homes->get_sellorders1($intid);
		echo json_encode($this->data['sellorderlist']);	
	}
}
public function loadmessagedata()
{
	$this->load->model('users');
	$this->data['userposts']=$this->homes->userposts();
	echo json_encode($this->data['userposts']);	
}
...
public function removeorder()
{
	 if( $this->input->is_ajax_request())
	 {
		 ...
		 if($orderid!='')
		 {
			 $removeorder=$this->homes->removeorder($orderid,$this->session->userdata['cc_user']['userid']);
			 echo $removeorder;
		 }
...
function loadactivedata()
{
	 ...
	 {
		 $intid=$this->input->post('id');
			    $activeorders=$this->homes->activeorders($intid);
...
function loadhistorydata()
{
	 ...
	 {
		 $intid=$this->input->post('id');
			    $activeorders=$this->homes->loadhistorydata($intid);
...
function getbalancefrom()
{
	 ...
	 {
		 $intid=$this->input->post('id');
		   $balance=$this->homes->getuserbalance($intid);
			echo ($balance);	
	 }
}
