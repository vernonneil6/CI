...
Class Homes extends CI_Model
{
	...
	function get_exchanges()
	{
		 ...
		 $this->db->order_by('interface_exchanges.ordering','asc');
		 $query=$this->db->get();
...
function get_buyorders($id)
{
	$where=array('interfaceexid'=>$id,
			'ordertype'=>'Buy',
			'order_status'=>0);
	$this->db->select('base_price,orderamount,ordertotal,order_id');
	...
	$this->db->where($where);
	 ...
	 $this->db->order_by('ordertotal','desc');
		$query = $this->db->get();
...
{		$where=array('interfaceexid'=>$id,
	...
	$this->db->where($where);
	 if(($this->session->userdata('interfaceid')!=''))
		 ...
		 $this->db->order_by('ordertotal','asc');
	$query = $this->db->get();
...
{
	  $query=$this->db->query("SELECT min(base_price) as base_price  FROM `user_orders` where `interfaceexid`='".$id."' and order_status=0 and ordertype='sell' order by createddate desc");
$d=$query->result_array();
if(!empty($d))
...
//get higest price
	...
	{
	  $query=$this->db->query("SELECT max(base_price) as base_price  FROM `user_orders` where `interfaceexid`='".$id."' and order_status=0 and ordertype='buy' order by createddate desc");
$d=$query->result_array();
if(!empty($d))
return $d[0]['base_price'];
	}
...
function activeorders($intid)
{
	...
	$this->db->select('base_price,orderamount,ordertotal,ordertype,createddate,order_id');
	$this->db->from('user_orders');
...
function get_userbalance($code)
{
	 $this->db->select('total_balance');
	$this->db->where('currencycode',$code);
...
{
	 $where=array('order_id'=>$orderid,'userid'=>$this->session->userdata['cc_user']['userid']);
	if($this->db->delete('user_orders',$where))
	{
		return 1;
	}
	else
...
{
	 $where=array('currencyid'=>$id,'userid'=>$this->session->userdata['cc_user']['userid']);
	$this->db->select('total_balance');
	$this->db->from('user_balance_main');
	$this->db->where($where);
	$query=$this->db->get();
