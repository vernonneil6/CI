...
Class Homes extends CI_Model
{
	...
	function get_last_price($id)
	{
		...
		if(($this->session->userdata('interfaceid')!=''))
		 {
			  echo $this->session->userdata('interfaceid');
			 $query=$this->db->query("SELECT (CAST( `base_price` AS decimal(6,3) )
) AS newprice FROM `user_orders` where `interfaceexid`='".$id."'  and order_status=0 and interfaceid='".$this->session->userdata('interfaceid')."' order by order_id desc limit 1");
	...
	//for buy currency
	function buycurrency($base_price,$buyamount,$buytotaltext,$feescalculationtext,$buybasecurrency,$interfaceexid,$interface)
	{
	...
	function sellcurrency($base_price,$buyamount,$buytotaltext,$feescalculationtext,$buybasecurrency,$interfaceexid,$interface)
	{
...
{
	 $sql="SELECT CAST( sum( `base_price` ) AS UNSIGNED ) AS totalbuy
...
{
	$sql="SELECT CAST( sum( `base_price` ) AS DECIMAL(10,4) ) AS totalsell
