...
function processGoToWebinar($webinar, $email)
{
    ...
    foreach ($_POST as $key => $value) {
        $key = strtolower($key);
        if (in_array($key, array('mc-firstname', 'firstname', 'first_name', 'first-name', 'fname', 'first', 'name', 'inf_field_firstname'))) {
            $firstName = $value;
            continue;
        }
        if (in_array($key, array('mc-lastname', 'lastname', 'last_name', 'last-name', 'lname', 'last', 'name', 'inf_field_lastname'))) {
            $lastName = $value;
        }
    }
...
function op_add_le_pages_filter($views)
{
    ...
    $views['live-editor'] = sprintf('<a href="%1$s"%2$s>%3$s <span class="count">(%4$d)</span></a>', admin_url('edit.php?post_type=page&le_page_filter=1&post_status=le'), $class, __('LiveEditor Pages', OP_SN), $count);
