...
function op_upload_field($fieldid,$value='',$return=false,$file_or_url='url',$disable_url=false){
    ...
    <div class="op-file-uploader">
        <a href="media-upload.php?post_id=0&amp;op_uploader=true'.($disable_url?'&amp;op_uploader_url_disable=Y':'').'&amp;TB_iframe=1" class="thickbox button" title="'.esc_attr__( 'Add Media' ).'">'.__('Select File',OP_SN).'</a>';
...
function op_thumb_gallery($fieldid, $value = '', $gallery_dir = '', $id = '', $return = false){
    ...
    <div id="'.$id.'" class="op-thumb-gallery">
        <a data-target="'.$id.'_gallery" href="#" class="button op-content-slider-button" title="'.esc_attr__( 'Select From Gallery' ).'">'.__('Select From Gallery',OP_SN).'</a>
    ';
    $html .= '
